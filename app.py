...
if SUPABASE_URL and SUPABASE_KEY:
    try:
        ...
        test_response = requests.get(f"{SUPABASE_URL}/rest/v1/elements?limit=1", headers=test_headers)
        print(f"🔍 اختبار Supabase: Status {test_response.status_code}")
        if test_response.status_code == 200:
            print("✅ Supabase متصل بشكل صحيح")
        elif test_response.status_code == 401:
            print("❌ خطأ في المصادقة: تأكد من SUPABASE_KEY (استخدم anon key)")
        elif test_response.status_code == 404:
            print("❌ الجدول غير موجود: قم بإنشاء جدول elements أولاً")
        else:
            print(f"⚠️ رد غير متوقع: {test_response.status_code}")
    except Exception as e:
        print(f"❌ فشل الاتصال بـ Supabase: {e}")
else:
    print("⚠️ Supabase غير مهيأ: SUPABASE_URL أو SUPABASE_KEY مفقود")
...
def admin_set_permission():
    ...
    # قفل الجدول مؤقتاً لمنع التداخل
        ...
        for element_data in elements_to_save:
            try:
                ...
                if response.status_code not in [200, 201, 204]:
                    ...
                    print(f"    URL: {SUPABASE_URL}/rest/v1/elements")
                    print(f"    Headers: {headers}")
                else:
                    print(f"✅ تم حفظ العنصر {element_data.get('element_id')} في Supabase")
            except Exception as e:
